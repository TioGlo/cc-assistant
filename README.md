...
The agent runs with `bypassPermissions` — full shell and filesystem — so who can talk to it matters:

- **Telegram** is locked to `telegram.owner_id` on every handler.
- **Discord** requires `discord.owner_ids`; with it unset, the bot ignores all messages (and logs why). The channel allowlist controls *where* the bot listens, not *who* may command it.
- **Slack** is a read-only triage feed: all messages in watched channels are summarized for you, but triage output can never execute SCHEDULE/DELEGATE blocks. Point it only at channels whose content you'd paste to the agent yourself.
- **Command-type SCHEDULE blocks** from model output are off by default (`scheduler.allow_command_blocks`) — see Cost engineering for why.
- **Model-initiated delegations** are budgeted (12/hour) to bound accidental DELEGATE loops; `/code` is exempt and blocked-budget events page you once.
...
- **Pure-bash scheduled jobs** (`scheduler.jobs[].command`) — for deterministic work that needs no judgment (ccusage reports, log rotation, disk checks, file cleanup, anything mechanical), set `command: "<bash>"` on the job *instead* of `prompt:`. The bot runs the command via `bash -c`, captures stdout/stderr, and routes the output through the same priority pipeline as prompt jobs (silent-logged by default — set `delivery: { priority: ... }` to push; failures always escalate). **Zero tokens.** `/jobs` distinguishes types with `🤖` (prompt) vs `⚙️` (command) icons. Optional `timeout_seconds` (default 60) and `silent_on_empty` (default true; suppress empty success outputs to avoid spam). Errors and timeouts are always reported. SCHEDULE blocks accept the same field, so the agent can create command-type jobs at runtime — but this is **gated behind `scheduler.allow_command_blocks` (default off)**: unlike the agent's interactive shell access, a persisted command job keeps running with no model in the loop, so a block injected via untrusted content the model read would be a durable backdoor. Blocked attempts page you (the command itself goes to the service log, not the notification). This is defense-in-depth on the *model-output* path, not a hard sandbox — an agent running under `bypassPermissions` can still write `scheduler-jobs.json` directly; tighten `allowed_tools`/`permission_mode` if that matters to you. Command jobs defined in config.yaml are always allowed.
