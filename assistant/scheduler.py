...
class Scheduler:
    ...
    def _append_dynamic_job(self, name: str, prompt: str, cron_expr: str, working_dir: str | None = None) -> None:
        jobs = self._load_dynamic_jobs()
        jobs = [j for j in jobs if j["name"] != name]
        jobs.append({"name": name, "prompt": prompt, "cron": cron_expr,
    ...
    def load_config_jobs(self, jobs: list[ScheduledJob]) -> None:
        ...
        for job in jobs:
            ...
            if job_id not in existing:
                self._add_to_scheduler(job.name, job.prompt, job.cron, job.working_dir, job_id_prefix="config_")
        ...
        for job in self._load_dynamic_jobs():
            ...
            if job_id not in existing:
                self._add_to_scheduler(job["name"], job["prompt"], job["cron"],
                                       job.get("working_dir"), job_id_prefix="user_")
                logger.info("Loaded dynamic job: %s (%s)", job["name"], job["cron"])

    # -- Public API --

    def add_cron_job(self, name: str, prompt: str, cron_expr: str,
                     working_dir: str | None = None, job_id_prefix: str = "user_") -> str:
        job_id = self._add_to_scheduler(name, prompt, cron_expr, working_dir, job_id_prefix)
        if job_id_prefix == "user_":
            self._append_dynamic_job(name, prompt, cron_expr, working_dir)
        return job_id

    def add_one_shot(self, prompt: str, delay: str, working_dir: str | None = None) -> str:
    ...
    async def _run_job(self, job_name: str, prompt: str, working_dir: str | None = None) -> None:
        logger.info("Executing scheduled job: %s", job_name)
        ...
        for attempt in range(max_retries):
            try:
                ...
                if session_id and "No conversation found" in str(e):
                    logger.info("Stale session, falling back to fresh")
                    session_id = None
